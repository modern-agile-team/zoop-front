...
// OpenAPI 스펙이 있는 URL
const SPEC_URL = 'https://lsh.taild0f974.ts.net/backend/swagger-json';

// 스펙을 저장할 파일 경로
const SPEC_WRITE_PATH = 'src/api/spec/index.json';
  ...
  return new Promise((resolve, reject) => {
    fetch(SPEC_URL)
      ...
      .then((data) => {
        ...
        // 해당하는 폴더가 없으면 생성
        const dirPath = SPEC_WRITE_PATH.split('/').slice(0, -1).join('/');
...
const results = await Promise.allSettled([getSpec()]);
